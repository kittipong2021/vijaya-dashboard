...
# ── 2. อ่านข้อมูล ──────────────────────────────────────────────────────────
def load_unit_data(hatchery, filepath):
    ...
    df_pl1 = pd.DataFrame(rows[1:], columns=rows[0])
    df_pl1 = df_pl1[df_pl1["Unit"].notna() & df_pl1["Tank #Crop"].notna()]
    # rename duplicate columns
    cols = list(df_pl1.columns)
    cols[12] = "Actual_density_L"
    cols[13] = "DoC_PL1"
    cols[14] = "pct_PL1"
    cols[15] = "TL_PL1"
    cols[16] = "pct_CV"
    ...
    def build_for_filter(dpu_unit, dru_unit, ptype):
        if ptype == "ALL":
            dpu_f, dru_f = dpu_unit, dru_unit
        else:
            dru_f = dru_unit[dru_unit["Type of Product"] == ptype]
            crops_in = set(dru_f["Crop"].dropna().unique())
            dpu_f = dpu_unit[dpu_unit["Crop"].isin(crops_in)]

        pl1g = dpu_f.groupby("Crop").agg(
            TL_PL1_mean=("TL_PL1", "mean"),
            density_mean=("Actual_density_L", "mean"),
        ).reset_index()
        rawg = dru_f.groupby("Crop").agg(
            TL_1st_mean=("TL-1st Pass QC", "mean"),
        ...
        if merged.empty:
            return None
        crops = [int(r) for r in merged["Crop"].tolist()]
        return {
            "crops":    crops,
            "TL_PL1":  clean(merged["TL_PL1_mean"].tolist()),
            "TL_1st":  clean(merged["TL_1st_mean"].tolist()),
            "density": clean(merged["density_mean"].tolist()),
    ...
    for unit in sorted(df_pl1["Unit"].dropna().unique()):
        ...
        result[unit] = {}
        for ptype in ["ALL", "PL4", "PL12"]:
            result[unit][ptype] = build_for_filter(dpu_u, dru_u, ptype)

    return result

print("กำลังอ่านข้อมูล Excel...")
ALL_DATA = {}
for hatchery, filepath in FILES.items():
    print(f"  {hatchery}: {filepath}")
    ALL_DATA[hatchery] = load_unit_data(hatchery, filepath)

if not any(ALL_DATA.values()):
    print("ERROR: ไม่พบไฟล์ Excel เลย กรุณาวางไฟล์ Excel ในโฟลเดอร์เดียวกัน")
    sys.exit(1)
...
.pl-btn{flex:1;height:20px;border:none;border-radius:4px;cursor:pointer;font-family:Tahoma,sans-serif;font-size:8px;font-weight:700;letter-spacing:.05em;transition:all .15s ease;border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.07);color:#b8c4a0}
...
var curH=Object.keys(DATA)[0]||'Vijaya1';
var curU=Object.keys(DATA[curH]||{})[0]||'A1';
var curPL='ALL';
...
function getData(){
  var h=DATA[curH]||{};
  var u=h[curU]||{};
  return u[curPL]||u['ALL']||{crops:[],TL_PL1:[],TL_1st:[],density:[],DOC_mean:[],DOC_min:[],DOC_max:[]};
}
...
function updateUI(){
  ...
  var stdEl=document.getElementById('std-label');
  if(stdEl)stdEl.textContent=curPL==='PL4'?'STD >=6.0':'STD >=7.5';
}
...
function selectHatchery(h,btn){
  if(!DATA[h])return;
  curH=h;curU=Object.keys(DATA[h])[0];
  document.querySelectorAll('.h-btn').forEach(function(b){b.classList.remove('active');});
  btn.classList.add('active');
  renderUnits();updateUI();initCharts();
}
function selectUnit(u,btn){
  curU=u;
  document.querySelectorAll('.u-btn').forEach(function(b){b.classList.remove('active');});
  btn.classList.add('active');
  updateUI();initCharts();
}
function selectPL(p,btn){
  curPL=p;
  document.querySelectorAll('.pl-btn').forEach(function(b){b.classList.remove('active');});
  btn.classList.add('active');
  updateUI();initCharts();
}
...
# แทนที่ placeholder ด้วยข้อมูลจริง (แก้ HTML entity ก่อน)
HTML = HTML.replace("&apos;", "'")
